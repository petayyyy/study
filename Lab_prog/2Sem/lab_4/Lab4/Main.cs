...
namespace Lab4
{
    public partial class Main : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            TotalOtputDGV.Columns[1].HeaderText = "Количество школ";
            TotalOtputDGV.Columns[1].Width = 100;
            TotalOtputDGV.RowCount = CountOfCompany;

            myOleDbCommand.CommandText = "SELECT [Код района] FROM [Место]";
            ...
            {
                ArForOut[i, 0] = Convert.ToInt32(myOleDbDataReader5["Код района"]);
                ArForOut[i, 1] = 0;
                i++;
            }
            ...
            {
                int t = Convert.ToInt32(myOleDbDataReader2["Код района"]);
                int g = Convert.ToInt32(myOleDbDataReader2["Год открытия"]);

                for (int k = 0; k < CountOfCompany; k++)
                {
                    if (g < 1917)
                    {
                        ArForOut[t-1, 1] ++;
            ...
            for (int h = 0; h < CountOfCompany; h++)
            {
